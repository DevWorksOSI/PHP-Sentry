...
class db
{
   ...
   /*
    * dbConnect
    * assigns variables from the sites config file (config.php)
    * @DBHOST
    * @DBUSER
    * @DBPASS
    * @DBNAME
   ...
   */
   public function confirm_query($result_set)
   {
      ...
      if (!$result_set) 
      {
         die("Database query failed." . $conn -> error); // Doesn't alway report the error
      }
   ...
   */
   ...
   {
      $data = mysqli_fetch_assoc($string);
      return $data;
   }
